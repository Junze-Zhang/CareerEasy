...
from CareerEasyBackend.careereasy_utils import am_i_a_match, extract_from_resume
...
#    profile_pic = data.get('profile_pic')
    ...
    new_account = CandidateAccount(username=username, email=email, password=encrypted_password.decode('utf8'), candidate=new_candidate)
    new_account.save()
...
def extract_candidate_info(request):
    ...
    candidate.standardized_ai_highlights = [STANDARDIZE_FN(highlight) for highlight in candidate.ai_highlights]
    candidate.highest_education = candidate_info["highest_education"]
    candidate.standardized_highest_education = STANDARDIZE_FN(candidate.highest_education)
    candidate.save()
...
def log_in(request):
    ...
    response.set_cookie(key='candidate_id', value=account.candidate.id, max_age=60 * 60 * 3)  # 3 hours
    response.set_cookie(key='candidate_account_id', value=account.id, max_age=60 * 60 * 3)  # 3 hours
    return response
...
def get_candidate_info(request):
    ...
    if account is None:
        return JsonResponse({"Error": "Session expired. Please log in again."}, status=401)
    return JsonResponse({"name": account.candidate.first_name + " "+ account.candidate.last_name,
                         "email": account.candidate.email,
                         "phone": str(account.candidate.phone),
                         "location": account.candidate.location,
                         "country": account.candidate.country,
                         "title": account.candidate.title,
                         "highlights": account.candidate.ai_highlights,
                         "skills": account.candidate.skills,
                         "highest_education": account.candidate.highest_education,
                         "resume": account.candidate.resume,
                         "preferred_career_types": [career.name for career in account.candidate.preferred_career_types.all()]}, status=200)
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(name='job_id', type=str, description='ID of the job to get details for')
    ],
    responses={
        200: OpenApiTypes.OBJECT,
        404: OpenApiTypes.OBJECT
...
def get_job_details(request, job_id):
    ...
    return JsonResponse({"Success": job}, status=200)

@extend_schema(
    tags=['Candidate Job Search'],
    description='Get detailed information about a specific company',
    parameters=[
        OpenApiParameter(name='company_id', type=str, description='ID of the company to get details for')
    ],
    responses={
        200: OpenApiTypes.OBJECT,
        404: OpenApiTypes.OBJECT
...
def get_company_details(request, company_id):
    ...
    return JsonResponse({"Success": company}, status=200)
